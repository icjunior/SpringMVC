...
public class FotoStorageS3 implements FotoStorage {
	...
	private static final String BUCKET = "brewerfotos";
	...
	public String salvar(MultipartFile[] files) {
		...
		if (files != null && files.length > 0) {
			...
			try {
				...
				throw new RuntimeException("Erro salvando no S3");
			}
		}
		return novoNome;
	}
	...
	private void enviarThumbnail(String novoNome, MultipartFile arquivo, AccessControlList acl) throws IOException {
		...
		amazonS3.putObject(new PutObjectRequest(BUCKET, THUMBNAIL_PERFIX + novoNome, is, thumbMetaData)
	...
	public byte[] recuperarThumbnail(String foto) {
		return recuperar(FotoStorage.THUMBNAIL_PERFIX + foto);
	}

	@Override
	public void excluirFoto(String foto) {
		amazonS3.deleteObjects(new DeleteObjectsRequest(BUCKET).withKeys(foto, THUMBNAIL_PERFIX + foto));
	}

	@Override
	public String getUrl(String foto) {
		if (!StringUtils.isEmpty(foto)) {
			return "https://brewerfotos.s3-sa-east-1.amazonaws.com/" + foto;
		}
		return null;
	}
}
